...
export default async function Footer() {
  ...
  const menuColumns = (settings.menu ?? []).slice(0, 4);

  const socialLinks = [
    {
      key: 'facebook',
      label: 'Facebook',
      url: normalizeUrl(socials?.facebook),
      icon: (
    ...
    },
    {
      key: 'linkedin',
      label: 'LinkedIn',
      url: normalizeUrl(socials?.linkedin),
      icon: (
    ...
    },
    {
      key: 'twitter',
      label: 'Twitter',
      url: normalizeUrl(socials?.twitter),
      icon: (
    ...
    <footer className="bg-secondary text-white pt-9 pb-5">
      <div className="container mx-auto">
        ...
        <div className="pt-5 border-t border-white text-xs">
          © {new Date().getFullYear()} {settings.title}. All rights reserved.
        </div>
      </div>
    </footer>
  );
